...
class InviteCommand extends LBCommand("invite", "Get an invite link for the bot, and the help server.", Permission.MESSAGE_SEND) {
  override def run(event: SlashCommandInteractionEvent): Unit = {
    ...
    eb.setImage(event.getJDA.getSelfUser.getEffectiveAvatarUrl)
    eb.setTitle("Anti-League Links")
    eb.setDescription(
      """
        |Need help with Anti-League? Join the help server here:
        |https://discord.gg/3Scnd3GvCn
        |
        |Want to invite Anti-League to your server? Click here:
        |https://discord.com/api/oauth2/authorize?client_id=967601110326050816&permissions=2052&scope=bot%20applications.commands
        |
